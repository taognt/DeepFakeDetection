...
def plot_figure( images, pred, gt, nrow,num_classes, categories=None, show=True):
    import matplotlib.pyplot as plt
    plot = make_grid(
        images, nrow, padding=4, normalize=False, scale_each=True, pad_value=1)
    if num_classes == 1:
    ...
    plt.axis("off")
    if show:
        plt.savefig(os.path.join("test_image.png"), dpi=300)
        plt.show()
        plt.close()
    else:
        plt.close()
